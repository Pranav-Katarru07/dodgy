import type { SpriteManifestEntry, SpriteState, BaseAnim, Effect, PokemonSpriteApi } from './types';
...
// ===========================================================================
// v0.4 sprite engine — RETAINED, @deprecated (Phase 4 deletion)
// ===========================================================================
//
// v0.4 consumers (src/gate/chase.ts, screens.ts, gate.ts, popup.ts) still
// import SpriteEngine + its statics/helpers and render the old single-row strip
// placeholder sheets in public/sprites/. Keep this class fully functional until
// Phase 3 swaps those consumers to PokemonSprite; Phase 4 deletes it.

/**
 * @deprecated v0.4 sprite engine. Superseded by {@link PokemonSprite}. Retained
 * so v0.4 consumers keep compiling AND rendering their placeholder sheets until
 * Phase 3 migrates them. Removed in Phase 4.
 */
...
/** @deprecated v0.4 only. See {@link PokemonSpriteOptions}. */
...
/**
 * @deprecated v0.4 frame picker (constant fps). Superseded by
 * {@link frameIndexForDurations}. Retained for the v0.4 engine below.
 */
...
/**
 * @deprecated v0.4 manifest resolver. Superseded by species-driven sheets.
 * Retained for the v0.4 engine below.
 */
export function resolveEntry(
  ...
  return (
    findEntry(manifest, tier, state) ??
    findEntry(manifest, 0, state) ??
    findEntry(manifest, 0, 'idle')
  );
}
...
type LegacyImageLike = ImageLike;

/**
 * @deprecated v0.4 sprite engine. Superseded by {@link PokemonSprite}. Retained
 * for green-tree compatibility until Phase 4.
 */
export class SpriteEngine {
  ...
  async load(tier: number): Promise<void> {
    ...
    for (const t of tier === 0 ? [0] : [tier, 0]) {
      for (const s of ALL_STATES) {
        const entry = findEntry(this.manifest, t, s);
  ...
  private isDrawable(img: LegacyImageLike): boolean {
